...
| Q11 | (re-verified) Are mob drop tables absent from the client sheets? | **Confirmed exhaustively.** The first answer checked four guessed names; this scanned **all 1,198 sheet types**. No `drop`/`loot`/`spoil`/`booty` type exists at all, every `reward`/`treasure` match is a scripted reward, and **every `BNpc*` sheet has zero item references** — a mob row cannot point at an item. The earlier `ItemDrop` hit was a substring of `ItemDropRate`, a levequest field. Settled; do not re-investigate. | 2026-08-26 |
