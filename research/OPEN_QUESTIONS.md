...
| Q17 | Can an arbitrary `ActionTimeline` row be played on the local player, for an animation the account does not own? | ❓ OPEN | 2026-08-28 | Asked by Trist. Target is row **13383** = `ornament_sp/m6017/onm_sp01` — a fashion-accessory animation, **not an emote**. Settled statically: it is absent from the `Emote` sheet, so `AgentEmote.ExecuteEmote` cannot reach it and `UIState.IsEmoteUnlocked` never applies; `TimelineContainer.PlayActionTimeline` takes an ActionTimeline row id and so has no ownership concept to check. What is NOT settled is whether the row's `Resident: true` means the .pap is only loaded while ornament 6017 is attached — `/mods/resolve` is a Penumbra echo and proves nothing about sqpack existence, and the brain exposes neither a file-exists check nor ornament state. Local-only display is expected and accepted; `IsMotionCanceledByMoving: true` means the game cancels on movement, also accepted. Probe built (`TimelineProbeWindow`, `/tchallenges anim`) — see TODO **R8**. |
