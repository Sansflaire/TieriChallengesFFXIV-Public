...
| 6 | Challenge types — the composite `InArea` kind: one or more areas, each with its own condition set (emote, mount, minion, full outfit, gear pieces N-of-M, NPC target, facing, game state, job+level, Eorzean time, carried item), in Single / any-order / in-order modes with per-step time budgets. Plus `RaceTimer`: an armed/running/ended state machine between a start and finish volume, with an optional time limit, an optional bounding area, restart-on-re-entry, and a persisted best time. Plus themes: Quest chains (steps that replace each other, only the chain completes) and Adventures (several objectives, progress persists), both derived from structure rather than authored, with a shared objective window. The five original kinds are retired but still evaluate forever. | 10 | ✅ done |
...
| 13 | Player-facing settings — sound volume/mute/per-cue toggles, notification toggles, popup duration, hold-in-combat, and a full colour palette editor | 1 | ✅ done — Settings window with Sound / Notifications / Colours tabs, reachable from both renderers |
| 14 | Browse controls — search, filter, sort in the challenge list | 1 | ✅ done — debounced search over both panes, a filter dropdown of type/state toggles, the difficulty ceiling, and three sort modes |
...
| 16 | User-facing help and onboarding + README | 1 | ✅ done — `docs/HELP.md` (39 sections, ~580 hidden search keywords) parsed at runtime by `HelpLibrary` and shown in a searchable Help window with click-to-jump; README refreshed; doc-maintenance rule in CLAUDE.md §6 |
