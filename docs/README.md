...
| [TODO](TODO.md) | **The checklist and the single source of status** for Challenge Tokens work. Permanent IDs (R/V/A/I), `⛔ Blocked by:` lines under every dependent item, a "startable right now" list, the critical path, and a Done log. Tick items here — the other three Token docs hold reasoning, not status. | 2026-08-26 |
| [Tokens Build Backlog](Tokens%20Build%20Backlog.md) | **Authoritative list of what does not exist yet** for Challenge Tokens: data pipeline, detection, client, Cloudflare server, web, rewards — with a critical path and cheap-wins section. Verified against the source tree, not memory. Add new build items here, not to Pending Verification. | 2026-08-26 |
| [Pending Verification](Pending%20Verification.md) | **Standing reminder list.** Everything Sansflaire still needs to test in-game, decide, or build for Challenge Tokens — plus what has already been answered. Re-read at the start of any Tokens/quests session; nothing leaves it until done. | 2026-08-26 |
