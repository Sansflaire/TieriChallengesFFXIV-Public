...
- [ ] **I54** 🙋 **Confirm the D3D11 city actually appears** (fix shipped 0.84.54.11, I53). Two
      blockers were fixed blind — a zero colour-write mask and a column-major matrix — and both are
      proven at the API level, but nobody has yet seen a pixel. In order, because the steps separate
      different failures: **(a)** occlusion **Off**, city visible and anchored to the ground while
      orbiting ⇒ both fixes landed; visible but sliding/warping ⇒ the matrix *source* is wrong, not
      its packing, and the next move is `Control.ViewProjectionMatrix` (what FFXIV-TV uses) — **not**
      reversing the multiply order. **(b)** switch to **D3D11 depth buffer**: terrain should hide
      buildings per pixel while roofs stay visible; everything vanishing ⇒ build the red/green
      rejection-map shader (Issues/017 "not changed"), which has no `discard` and so cannot lie by
      omission. **(c)** bias tuning last, never as a substitute for (b).
- [ ] **I55** 🤖 Decide the **premultiplied-alpha** question, but only if I54 passes and the grid
      looks too faint. The PS returns `float4(col.rgb * col.a, col.a)` and ImGui then blits the
      surface; if Dalamud's backend blends straight-alpha, alpha is applied twice — dimming the grid
      (0.55) and checker (0.16) and leaving walls (1.0) untouched. Deliberately unread rather than
      guessed. Test: the opacity slider at 0.5 should look half, not a quarter.
      ⛔ Blocked by: **I54**
