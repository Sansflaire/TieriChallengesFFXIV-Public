...
internal sealed class DigTestsWindow
{
    ...
    /// <summary>
    /// The clue glossary, in a popup so it can be read beside a clue without losing the lab's place.
    ///
    /// <para>Its content is generated by <c>DigClueWriter.Vocabulary()</c>, which calls the real
    /// helpers rather than restating them — so the words here are literally the words the writer
    /// would emit, and cannot drift from it.</para>
    /// </summary>
    private static bool _vocabOpen;

    /// <summary>
    /// The clue glossary, in a WINDOW rather than a popup.
    ///
    /// <para><b>A popup collapsed to a strip.</b> <c>BeginPopup</c> auto-sizes to its content, and a
    /// <c>BeginChild</c> asking for "-1" height inside an auto-sizing parent resolves to nothing —
    /// each was waiting for the other to declare a size. <c>SetNextWindowSize</c> on a popup does
    /// not settle it either, because the auto-size is recomputed from content.</para>
    ///
    /// <para>A window is the right container regardless: this is a reference to be read BESIDE a
    /// clue, and a popup closes the moment you click anything else — including the very clue you
    /// opened it to interpret.</para>
    ///
    /// <para>Drawn from the lab's top level, not from inside the clue-categories block, so
    /// collapsing that section does not take the window with it.</para>
    /// </summary>
    private static void DrawVocabularyWindow()
    {
        if (!_vocabOpen) return;

        ImGui.SetNextWindowSize(new Vector2(760f, 620f), ImGuiCond.FirstUseEver);

        if (ImGui.Begin("Clue vocabulary (DEV ONLY)", ref _vocabOpen))
        {
            ImGui.TextColored(Rule,
                "Generated by calling the clue writer's own functions, not by restating them — so\n"
              + "these are literally the words it emits and cannot fall out of step with it.");

            ImGui.Separator();

            foreach (string line in DigClueWriter.Vocabulary())
            {
                if (line.Length == 0) { ImGui.Spacing(); continue; }

                // Unindented lines are headings; everything else is detail beneath one.
                if (line[0] != ' ') ImGui.TextColored(Head, line);
                else                ImGui.TextUnformatted(line);
            }
        }

        // Outside the if, and unconditionally: ImGui requires End for every Begin regardless of
        // what it returned, and a missed one corrupts every window drawn afterwards.
        ImGui.End();
    }
    ...
    private void DrawRoamPlacement()
    {
        ...
        ImGui.TextColored(Rule,
            "DIFFICULTY IS A POINT BUDGET over how many facts and how vague each one is.\n"
          + "  HOW MANY FACTS, by band:\n"
          + "    0.00–0.33  EASY    3 facts\n"
          + "    0.34–0.66  MEDIUM  2 facts\n"
          + "    0.67–1.00  HARD    1 fact\n"
          + "  EVERY FACT HAS FOUR TIERS OF VAGUENESS (1–4) plus the plain phrasing at 0. One tier\n"
          + "  is one point, spread over randomly chosen facts — so one fact can be woolly while the\n"
          + "  others stay sharp, rather than all three degrading together.\n"
          + "  MANDATORY VAGUENESS: more facts is inherently easier, so more facts must each be put\n"
          + "  less sharply before anything is charged. 3 facts floor at 1/1/2, 2 facts at 1/1, a\n"
          + "  single fact has no floor. Those floors are ZEROED — two facts at vagueness 1 apiece\n"
          + "  is difficulty ZERO — so every band's easiest setting is equally easy.\n"
          + "  Ceilings therefore differ: 8 for three facts, 6 for two, 4 for one.\n"
    ...
    private void DrawClueCategories()
    {
        ...
        if (ImGui.Button("Clue vocabulary…")) _vocabOpen = true;

        ImGui.SameLine();
        ImGui.TextDisabled("every phrase a clue can use, and what each one means exactly");
