...
internal sealed class DigTestsWindow
{
    ...
    /// </summary>
    private static void DrawVocabularyPopup()
    {
        ImGui.SetNextWindowSize(new Vector2(720f, 560f), ImGuiCond.Appearing);

        if (!ImGui.BeginPopup("##cluevocab")) return;

        ImGui.TextColored(Head, "WHAT EVERY CLUE PHRASE MEANS");
        ImGui.TextColored(Rule,
            "Generated from the clue writer itself — the compass points, distance words and grid\n"
          + "cells below are produced by calling the real functions, not by restating them, so this\n"
          + "cannot fall out of step with what the generator actually says.");

        ImGui.Separator();

        if (ImGui.BeginChild("##vocabscroll", new Vector2(0f, -1f), false))
        {
            foreach (string line in DigClueWriter.Vocabulary())
            {
                if (line.Length == 0) { ImGui.Spacing(); continue; }

                // Headings are the unindented lines; everything else is detail under one.
    ...
    private void DrawClueCategories()
    {
        ...
        if (ImGui.Button("Clue vocabulary…")) ImGui.OpenPopup("##cluevocab");
